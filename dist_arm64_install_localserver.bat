...
set "SOURCE_TSF_SERVER=%SCRIPT_DIR%\TigerClaw.TsfServer.exe"
...
set "TARGET_TSF_SERVER=%TARGET_DIR64%\TigerClaw.TsfServer.exe"
...
set "FAILED=0"
set "LOCKED=0"
set "TIGERCLAW_ARM64_REGISTER_DIRECT=0"

call :log "Install TigerClaw Windows on Arm package (LocalServer32 diagnostic)"
echo ====================================
echo Install TigerClaw for Windows on Arm - LocalServer32 diagnostic
echo ====================================
call :check_arm64_os || goto :failed
for %%P in ("%SOURCE_WRAPPER%" "%SOURCE_ARM64%" "%SOURCE_X64%" "%SOURCE_X86%" "%SOURCE_TSF_SERVER%" "%CORE_EXE%" "%OVERLAY_EXE%" "%DIALOG_EXE%" "%SHARED_DLL%") do if not exist %%~P echo [Error] Missing %%~P & call :log "Missing %%~P" & set "FAILED=1"
...
call :unregister_localserver
call :unregister_if_exists "ARM64X" "%REGSVR64%" "%TARGET_WRAPPER%"
call :unregister_if_exists "ARM64 direct" "%REGSVR64%" "%TARGET_ARM64%"
call :unregister_if_exists "Win32" "%REGSVR32%" "%TARGET_X86%"

echo [3/6] Delete installed DLLs
for %%F in ("%TARGET_WRAPPER%" "%TARGET_ARM64%" "%TARGET_X64%" "%TARGET_TSF_SERVER%" "%TARGET_X86%") do (
  call :delete_if_exists "%%~F" || (set "FAILED=1" & set "LOCKED=1")
)
if "!FAILED!"=="1" goto :failed

echo [4/6] Copy and register DLLs
echo [Info] 64-bit TSF COM will be registered as LocalServer32 via TigerClaw.TsfServer.exe.
...
copy /Y "%SOURCE_TSF_SERVER%" "%TARGET_TSF_SERVER%" >nul || set "FAILED=1"
copy /Y "%SOURCE_X86%" "%TARGET_X86%" >nul || set "FAILED=1"
if "!FAILED!"=="1" goto :failed
call :register_localserver || set "FAILED=1"
...
call :verify_registry_contains "LocalServer32" "HKCR\CLSID\%TIP_CLSID%\LocalServer32" "%TARGET_TSF_SERVER%" || set "FAILED=1"
...
call :popup "TigerClaw" "LocalServer32 diagnostic install completed. Test ARM64/UWP/x64 activation and input." "Info"
exit /b 0
...
if not exist "%TARGET_TSF_SERVER%" exit /b 1
"%TARGET_TSF_SERVER%" --register-localserver
exit /b %errorlevel%

:unregister_localserver
if exist "%TARGET_TSF_SERVER%" "%TARGET_TSF_SERVER%" --unregister-localserver >nul 2>&1
exit /b 0

:verify_registry_contains
set "ACTUAL_PATH="
