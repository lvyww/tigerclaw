...
namespace tiger::overlay
{
    ...
    class Application
    {
        ...
        Text currentSchema_;
        bool menuPending_ = false, menuOpen_ = false, layoutPending_ = false;
        ...
        void Menu(bool refresh = true)
        {
            if (menuOpen_) return;
            if (!demo_ && refresh)
            {
                if (!menuPending_) menuPending_ = commands_->Send("get_schema_list", 3);
                return;
            }
            const auto menuSchemas = schemas_;
            ...
            for (std::size_t i = 0; i < menuSchemas.size() && i < 1000; ++i)
                AppendMenuW(schemas, MF_STRING | (menuSchemas[i] == currentSchema_ ? MF_CHECKED : 0), 100 + i, Wide(menuSchemas[i]).c_str());
            if (menuSchemas.empty()) AppendMenuW(schemas, MF_STRING | MF_GRAYED, 0, L"\u6682\u65e0\u65b9\u6848");
            ...
            if (choice >= 100 && choice < 1100 && static_cast<std::size_t>(choice - 100) < menuSchemas.size())
                commands_->SetConfig(u"\u5f53\u524d\u7801\u8868", menuSchemas[choice - 100]);
        ...
        LRESULT Message(HWND hwnd, UINT message, WPARAM wp, LPARAM lp)
        {
            ...
            case WM_TIMER: Refresh(false); return 0;
            case StateMessage:
            ...
            case ReplyMessage:
                if (commands_)
                {
                    ...
                    while (commands_->Take(reply))
                    {
                        ...
                        if (!reply.success)
                        {
                            if (reply.tag == 3) { menuPending_ = false; Menu(false); }
                            continue;
                        }
                        auto json = nlohmann::json::parse(reply.json);
                        if (reply.tag == 1 || reply.tag == 3)
                        {
                            schemas_.clear();
    ...
    public:
        ...
        int Run()
        {
            ...
            if (!candidate_ || !status_) throw std::runtime_error("Window creation failed");
            ...
            else
            {
                ...
                commands_->Send("get_schema_list", 1);
