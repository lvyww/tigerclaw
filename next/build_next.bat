...
echo [0/5] Stop running next processes (if any)
for %%P in (TigerClaw.Core.exe TigerClaw.Overlay.exe TigerClaw.Dialog.exe TigerClaw.Hook.Native.exe) do (
  taskkill /F /IM %%P >nul 2>nul
)

echo [1/5] Build TigerClaw.Shared
"%DOTNET%" msbuild "%~dp0TigerClaw.Shared\TigerClaw.Shared.csproj" /restore /p:Configuration=Debug /m || exit /b 1

echo [2/5] Build TigerClaw.Core (OutDir -> _run)
"%DOTNET%" msbuild "%~dp0TigerClaw.Core\TigerClaw.Core.csproj" /restore /p:Configuration=Debug /p:OutDir="%UNIFIED_OUT%\\" /m || exit /b 1

echo [3/5] Build TigerClaw.Overlay (OutDir -> Core)
...
echo [4/5] Build TigerClaw.Dialog (OutDir -> Core)
...
if exist "%HOOK_NATIVE_PROJECT%" (
  echo [5/5] Build TigerClaw.Hook.Native (OutDir -> native)
