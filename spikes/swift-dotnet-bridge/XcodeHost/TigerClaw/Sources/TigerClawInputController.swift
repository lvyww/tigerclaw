...
final class TigerClawInputController: IMKInputController {
    ...
    override func handle(_ event: NSEvent!, client sender: Any!) -> Bool {
        ...
        return process(key: key, client: sender)
    }

    override func inputText(_ text: String!, key keyCode: Int, modifiers _: Int, client sender: Any!) -> Bool {
        let key = keyCode == 49 ? "space" : (text ?? "").lowercased()
        trace("inputText key=\(key) client=\(sender is IMKTextInput) engine=\(engine != nil)")
        guard key.count == 1 || key == "space" else { return false }
        return process(key: key, client: sender)
    }

    override func inputText(_ text: String!, client sender: Any!) -> Bool {
        let key = (text ?? "").lowercased()
        trace("inputTextSimple key=\(key) client=\(sender is IMKTextInput) engine=\(engine != nil)")
        guard key.count == 1 || key == " " else { return false }
        return process(key: key == " " ? "space" : key, client: sender)
    }

    private func process(key: String, client sender: Any!) -> Bool {
