...
候选列表只显示尚未确认的后半段，不重复暂存前缀；只剩暂存文字时隐藏候选列表，
仅保留预编辑文字，空格仍可提交。继续输入编码后恢复候选列表。
内部标记的标点候选由过滤器排除，Tab/方向键不能在文字暂存边界选中它。
提交前的通知负责合并前缀与所选后缀，仍是一次完整提交。
有暂存文字时按逗号等标点，先完整提交暂存前缀与当前候选，再按用户标点表处理按键，
避免标点分段后遗留编码或颠倒文字。真实模型回归覆盖 `kospfify` 加逗号输出“测试一下，”。
关闭提前上屏时，这个选项本身不会触发提前确认。

退格先删除剩余编码；编码删空后逐个 Unicode 字符删除暂存文字，不恢复原始编码。
多字词也可以只删除最后一个字，后续解码不能把已删文字带回来。
只剩文字时，每次删除都会使 Rime 重新生成预编辑，避免内部编码不变导致旧文字残留。
回车提交暂存文字与剩余原始编码；Esc、外部取消和切换方案取消整个暂存组合。
中英文切换若原绑定为提交编码或行内英文，在有暂存文字时改为提交当前候选；
原绑定为清空则继续清空。没有暂存文字时保留原来的切换行为。
中途关闭本选项不会丢弃已暂存文字，仍等待最终确认。学习只在最终宿主提交后发生。

显示位置由前端决定，可能是候选窗输入栏，也可能是应用光标处的预编辑区。
内部使用单个 `~` 保持只剩文字时的组合，Lua 不把它送入解码、预编辑显示或候选提交。
请同时更新 `rime.lua`、两个 Lua 文件、主 schema 和新增的
`tiger_sentence_ascii.schema.yaml`。后者是内部切换配置，不加入 schema_list。
注册项新增 `tiger_sentence_ascii = tiger_sentence.ascii_component`。
同时注册 `tiger_sentence_buffer_filter = tiger_sentence.buffer_filter`，并更新主 schema 中的过滤器配置。

真实宿主验证：编译 `tools/rime_preedit_probe.cpp`（链接 `-lrime -ldl`），通过
`tools/test_rime_preedit_integration.py --exe <探针> --plugin <librime-lua.so>` 运行。
覆盖暂存、多字词逐字删除、空格/点选、回车、Shift/Shift+Tab、Caps、方案切换和延迟学习。
概率路径另由真实模型回归覆盖；这些测试不代替小狼毫/同文的实际显示验收。

锁定前缀使用独立的单份增量缓存：同一代候选与置信度共用路径，追加编码只扩展
新尾部；前缀文字、边界、码表、模型、学习快照或重码规则改变时失效。
回归逐步对比追加、回删、编辑后的缓存结果与完整重建结果。
无界面按键基准可使用上述真实宿主脚本的 `--production-model <模型>`、
`--benchmark-raw <编码>` 和 `--benchmark-mode buffer|application`，输出三轮逐键耗时。
...
纯 Lua 没有后台工作线程：数据库写入仍在明确提交时完成，成功后只更新受影响的
编码分区，不重放全部历史。新组合按分钟更新评分时间，查询时才计算对应编码的
衰减和前缀索引；旧组合持有的快照不变。未来时间戳或系统时钟回退会走完整重放
以保持评分一致。文件格式、数据库散列与原版兼容，不需要清空已有学习记录。
大量记录下的手机数据库写入延迟仍需实测。
...
码表解析后复用已去重的源顺序，频序按已有稠密排名筛选，编码长度先去重再排序。
加载不增加磁盘缓存，修改 TXT 后重新部署的方式不变。规则散列加速保持原字节结果。
置信度复用前缀聚合记录，合并候选仅在概率质量碰撞时复制；仍使用完整 Beam，
不缩小搜索宽度或置信度池。模型页缓存使用数值页号，容量仍为 8 MiB。
锁定属性未变时复用解析结果，列表各自独立；候选刷新复用已裁切的编码后缀，
暂存文字仍取当前状态，连续回删不会显示旧文字。

测试与测量见 `tools/RIME_PERFORMANCE.md`。学习 CPU 基准为
`lua tools/bench_rime_learning.lua rime/tiger_sentence/lua 10`；它使用内存存储替身，
不包含真实数据库写入或前端绘制时间。
