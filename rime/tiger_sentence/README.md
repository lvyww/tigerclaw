...
- `提前上屏` 默认开启，可从方案选单关闭。开启后，原始编码超过 4 键时，同一稳定前缀必须在两个严格连续的一键追加上达到 0.995 置信度；置信度覆盖完整的 200 状态最终 beam，并合并同文不同路径的概率质量，不再只看窗口中的 20 个候选；如果最终 beam 已被裁剪，本轮不会提前提交
- 每次至少提前提交两个新字、距离上次提交至少新增三个编码键，并始终保留候选最后一个字；退格会清空稳定证据，手动上下翻选或 Tab 遍历候选后会暂停本次 composition 的提前上屏
