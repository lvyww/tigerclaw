...
- `提前上屏` 默认开启，可从方案选单关闭。开启后，原始编码超过 4 键时，取三个严格连续一键追加代各自达到 0.995 置信度的最长提案之公共前缀，并要求该前缀在三代中对应相同的原始编码终点；置信度覆盖完整的 200 状态最终 beam，并合并同文不同路径的概率质量，不再只看窗口中的 20 个候选；如果最终 beam 已被裁剪，本轮不会提前提交
