"""Compressed fivegram Beam scoring on frozen old/fresh shape cases. Accuracy only."""
...
out=Path('/mnt/c/Archive/tigerclaw_sentence_ml/experiments/brightmart-char5-500mb-20260922/evaluation');out.mkdir(exist_ok=True);local=Path('/home/yc/tmp/tiger-shape-direct5');fixture=local/'fixture'
selected=json.loads((out.parent/'selected.json').read_text())
...
env=dict(os.environ,SHAPE5_LIB=str(local/'shape5.so'),SHAPE5_MODEL=selected['local_model'])
def run(i):
...
rows=[json.loads(l) for l in (root/'direct5/predictions-merged.jsonl').read_text().splitlines()]
...
for r in rows:
 ...
 r['predictions']['budget_fivegram']=pool[0]['text'];r['budget_fivegram_rank']=next((int(c['index']) for c in pool if c['text']==r['target']),0)
...
for dataset in ('old','fresh','combined'):
 ...
 for k in (5,20):
  before=sum(0<r['rank']<=k for r in subset);after=sum(0<r['budget_fivegram_rank']<=k for r in subset)
  gained=[r['id'] for r in subset if not 0<r['rank']<=k and 0<r['budget_fivegram_rank']<=k]
  lost=[r['id'] for r in subset if 0<r['rank']<=k and not 0<r['budget_fivegram_rank']<=k]
  data['pool'][str(k)]=dict(trigram=before,fivegram=after,gained=gained,lost=lost)
 data['comparisons']={}
 for baseline in ('baseline','installed_fused3','fivegram_top5','fusion_half_top5','direct_fivegram'):
  rescued=sum(r['predictions']['budget_fivegram']==r['target'] and r['predictions'][baseline]!=r['target'] for r in subset)
  regressed=sum(r['predictions']['budget_fivegram']!=r['target'] and r['predictions'][baseline]==r['target'] for r in subset)
  data['comparisons'][baseline]=dict(rescued=rescued,regressed=regressed,net=rescued-regressed)
 summary[dataset]=data
for baseline in ('baseline','installed_fused3','fivegram_top5','fusion_half_top5','direct_fivegram'):
 changed=[dict(id=r['id'],source=r['source'],code=r['code'],target=r['target'],before=r['predictions'][baseline],after=r['predictions']['budget_fivegram'],rescued=r['predictions']['budget_fivegram']==r['target'],regressed=r['predictions'][baseline]==r['target']) for r in rows if (r['predictions'][baseline]==r['target'])!=(r['predictions']['budget_fivegram']==r['target'])]
 (out/(baseline+'-changes.json')).write_text(json.dumps(changed,ensure_ascii=False,indent=2)+'\n')
(out/'summary.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2)+'\n')
def digest(p):
 with p.open('rb') as f:return hashlib.file_digest(f,'sha256').hexdigest()
manifest=dict(cases_sha256=digest(root/'cases.tsv'),model_sha256=validation['model_hash'],native_sha256=digest(local/'shape5.so'),patched_decoder_sha256=digest(fixture/'lua/tiger_sentence.lua'),baseline_decoder_sha256=digest(Path('/home/yc/tmp/tiger-char5-shape-20k/fixture/lua/tiger_sentence.lua')),validation=validation,method='Fivegram full opaque state propagated on every Beam path, per-character expansion and EOS. Same frozen Beam limits/eligibility/rank/word priors. Trigram retained only for original observed-bigram isolation prior, never for search LM probability.',latency_evaluated=False,llm=False,production_changed=False,compressed_model=selected)
(out/'manifest.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2)+'\n')
for filename in ('shape5_lua.cpp','patch_shape5.py','test_shape5.lua','evaluate_shape_budget5.py'):shutil.copy2(Path(__file__).with_name(filename),out/filename)
