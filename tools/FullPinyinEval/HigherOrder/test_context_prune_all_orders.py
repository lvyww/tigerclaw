...
import sys
pruner=Path(sys.argv[1]);builder=Path(sys.argv[2])
def read(p):
 d={};n=0
...
with tempfile.TemporaryDirectory(prefix='tiger-context-prune-') as td:
 ...
 for n in range(2,6):
  ...
  for h in itertools.product(('a','b'),repeat=n-1):
   weights=((.6,.3,.1) if h[0]=='a' else (.3,.6,.1)) if n==3 else ((.55,.35,.1) if h[0]=='a' else (.35,.55,.1)) if n==2 else (((.65,.25,.1) if h[0]=='a' else (.35,.55,.1)) if n==4 else ((.7,.2,.1) if h[0]=='a' else (.2,.7,.1)))
 ...
 subprocess.run([str(pruner),str(p/'in.arpa'),str(p/'small.arpa'),str(p/'all.arpa'),'1:1:1:1,2:2:2:2'],check=True)
 ...
 for n in range(5):
  for h in itertools.product(('a','b'),repeat=n):
   ...
   if n>=1:
    for t in ('a','b','</s>'):
     effective=h
     while len(effective)>=1 and any(x!='a' for x in effective):effective=effective[1:]
     expected=prob(original,effective,t)
     assert abs(prob(small,h,t)-expected)<1e-9
 subprocess.run([str(builder),'trie',str(p/'small.arpa'),str(p/'small.klm')],check=True,stdout=subprocess.DEVNULL)
 (p/'broken.arpa').write_text((p/'in.arpa').read_text().split('\\4-grams:')[0])
 bad=subprocess.run([str(pruner),str(p/'broken.arpa'),str(p/'bad1.arpa'),str(p/'bad2.arpa'),'1:1:1:1,2:2:2:2'],capture_output=True)
