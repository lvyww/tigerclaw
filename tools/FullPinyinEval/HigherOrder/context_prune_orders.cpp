...
// Keep all orders <=2; order-specific suffix-closed thresholds. Enable higher-order distributions only when every
...
int main(int argc,char**argv){try{
 if(argc!=5)throw std::runtime_error("input.arpa output1.arpa output2.arpa thresholds3:4:5,thresholds3:4:5");
 ...
 for(int j=0;j<2;j++){
  ...
  for(int n=3;n<=5;n++){if(!std::getline(parts,part,':'))throw std::runtime_error("need 3:4:5 thresholds");limit[j][n]=std::stoi(part);if(limit[j][n]<0 || (n>3&&limit[j][n]>limit[j][n-1]))throw std::runtime_error("thresholds must be nonnegative and nonincreasing");}
 }
 ...
 while(std::getline(in,line)){
  ...
  if(n>=2)for(int k=1;k<=n;k++){auto it=ranks.find(f[k]);if(it==ranks.end())throw std::runtime_error("unknown token");fullMax=std::max(fullMax,it->second);if(k<n)historyMax=fullMax;}
  for(int j=0;j<2;j++){
   bool keep=n<=2||historyMax<=limit[j][n];if(!keep)continue;
   counts[j][n]++;
   if((n>=2&&n<5)&&fullMax>limit[j][n+1]){
