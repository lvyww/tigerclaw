...
with tempfile.TemporaryDirectory(prefix='tiger-context-prune-') as td:
 ...
 for n in range(2,6):
  ...
  for h in itertools.product(('a','b'),repeat=n-1):
   weights=((.6,.3,.1) if h[0]=='a' else (.3,.6,.1)) if n==3 else (.5,.4,.1) if n<3 else (((.65,.25,.1) if h[0]=='a' else (.35,.55,.1)) if n==4 else ((.7,.2,.1) if h[0]=='a' else (.2,.7,.1)))
 ...
 subprocess.run([str(pruner),str(p/'in.arpa'),str(p/'small.arpa'),str(p/'all.arpa'),'1:1:1,2:2:2'],check=True)
 ...
 for n in range(5):
  for h in itertools.product(('a','b'),repeat=n):
   ...
   if n>=2:
    for t in ('a','b','</s>'):
     effective=h
     while len(effective)>=2 and any(x!='a' for x in effective):effective=effective[1:]
 ...
 bad=subprocess.run([str(pruner),str(p/'broken.arpa'),str(p/'bad1.arpa'),str(p/'bad2.arpa'),'1:1:1,2:2:2'],capture_output=True)
