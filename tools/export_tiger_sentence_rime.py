...
def write_lua_lexicon(path: Path, filtered: Dict[str, List[Tuple[str, int]]]) -> None:
    ...
    for stale in path.parent.glob(path.stem + "_[0-9][0-9].lua"):
        stale.unlink()

    for start in range(0, len(ordered_codes), shard_size):
        shard_number = len(shard_names) + 1
        shard_name = "%s_%02d" % (path.stem, shard_number)
        shard_names.append(shard_name)
        shard_lines = [
            "-- Generated by tools/export_tiger_sentence_rime.py. Do not edit.",
            "return {",
        ]
        for code in ordered_codes[start:start + shard_size]:
            items = ", ".join(
                '{t="%s",r=%d}' % (lua_escape(text), rank)
                for text, rank in filtered[code]
            )
            shard_lines.append('  ["%s"] = {%s},' % (lua_escape(code), items))
        shard_lines.extend(["}", ""])
        (path.parent / (shard_name + ".lua")).write_text(
    ...
    for length in lengths:
        lines.append("  [%d] = true," % length)
    lines.extend([
        "}",
        "local shards = {",
    ])
    for shard_name in shard_names:
        lines.append('  require("%s"),' % shard_name)
    lines.extend([
        "}",
...
CUSTOM_LUA_TEMPLATE = """-- 用户自定义编码。修改后重新部署 Rime。
...
def ensure_custom_lua(path: Path) -> None:
    if not path.is_file():
        path.write_text(CUSTOM_LUA_TEMPLATE, encoding="utf-8")
...
def main() -> int:
    ...
    ensure_custom_lua(args.output / "lua" / "tiger_sentence_custom.lua")
