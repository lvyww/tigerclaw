...
for %%P in ("%CORE_PROJECT%" "%OVERLAY_PROJECT%" "%DIALOG_PROJECT%" "%HOOK_PROJECT%" "%TSF_PROJECT%" "%TSF_SERVER_PROJECT%" "%WRAPPER_DIR%\build.bat" "%INSTALL_TEMPLATE%" "%INSTALL_DIRECT_TEMPLATE%" "%INSTALL_ARM64X_TEMPLATE%" "%INSTALL_LOCALSERVER_TEMPLATE%" "%DIAGNOSE_TEMPLATE%" "%UNINSTALL_TEMPLATE%" "%UPDATE_EMBED_SCRIPT%") do if not exist %%~P echo ERROR: Missing %%~P & exit /b 1
...
for %%P in ("%CORE_OUT%\TigerClaw.Core.exe" "%CORE_OUT%\TigerClaw.Overlay.exe" "%CORE_OUT%\TigerClaw.Dialog.exe" "%CORE_OUT%\TigerClaw.Shared.dll" "%HOOK_OUT%\TigerClaw.Hook.Native.exe" "%TSF_X86%" "%TSF_X64%" "%TSF_ARM64%" "%TSF_SERVER%" "%WRAPPER_DLL%") do if not exist %%~P echo ERROR: Missing %%~P & exit /b 1
...
if exist "%CORE_OUT%\TigerClaw.Overlay.exe.config" copy /Y "%CORE_OUT%\TigerClaw.Overlay.exe.config" "%RELEASE_DIR%\TigerClaw.Overlay.exe.config" >nul || exit /b 1
