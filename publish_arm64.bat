...
for %%P in ("%CORE_PROJECT%" "%OVERLAY_PROJECT%" "%DIALOG_PROJECT%" "%SENTENCE_PROJECT%" "%HOOK_PROJECT%" "%TSF_PROJECT%" "%TSF_SERVER_PROJECT%" "%WRAPPER_DIR%\build.bat" "%INSTALL_TEMPLATE%" "%INSTALL_DIRECT_TEMPLATE%" "%INSTALL_ARM64X_TEMPLATE%" "%INSTALL_LOCALSERVER_TEMPLATE%" "%DIAGNOSE_TEMPLATE%" "%UNINSTALL_TEMPLATE%" "%UPDATE_EMBED_SCRIPT%") do if not exist %%~P echo ERROR: Missing %%~P & exit /b 1
...
"%DOTNET%" msbuild "%SENTENCE_PROJECT%" /restore /p:Configuration=Release /p:Platform=x64 /p:OutDir="%SENTENCE_OUT%\\" /v:minimal || exit /b 1
