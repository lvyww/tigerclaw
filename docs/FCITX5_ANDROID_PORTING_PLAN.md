...
2026-09-13：同步 Tab 纠正学习。`TabLearning` 默认开启，按双方路径共同边界提取
纠正片段，保留最多四条学习前缀路径；学习影响过的搜索结果禁止提供自动上屏证据。
评分、衰减、竞争偏好和前文隔离参考虎娘 C++，前缀索引采用 Windows 后续修复的
预聚合方式。用户目录 `.tigerclaw-learning-v1.log` 由工作线程加锁读写；只有
`commitString` 提交后匹配且未消费的本地回执才进入队列。这是 Fcitx5 宿主提交，
不等于目标应用插入确认。主机回归和 ARM64 原生插件编译与手机实测分别记录。
2026-09-14：同一开关也控制直接点选学习。点选非首选与当前首选比较，首选不强化；
Tab 后点选和重复提交回执只记录一次。点选上屏前不发布学习偏好。
