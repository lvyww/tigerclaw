...
- Core sentence early commit is experimental and controlled by `整句自动提前上屏` (default off). The authoritative raw-code length gate is strict: raw length `<= 4` never triggers or accumulates stability; only after the fifth raw key do all candidates (including one-/two-key first segments) participate normally. Stability requires the same confidence prefix twice, confidence mass `>= 0.995`, and the final candidate character is retained. Decoder results, candidate display, completion, and suffix commits all enforce that a candidate must retain `_sentenceCommittedText`; conflicting async results cannot be submitted. `_sentenceCommittedRawLength` maps committed text to complete code edges, so display spaces are removed only from the live tail. Keep this invariant when changing early-commit logic; do not permanently filter short first segments after the raw-length gate.
...
Rime early-commit work is experimental and has not been validated in Fcitx/iOS clients. It is exposed as the `tiger_sentence_early_commit` schema switch and defaults off. `rime/tiger_sentence/lua/tiger_sentence.lua` keeps committed text/raw length plus one packed confidence property in the Rime context (not a Lua table keyed by temporary context userdata), decodes the complete committed-prefix plus live-tail raw code, filters candidates to the committed prefix, commits a stable prefix after raw length > 4, and trims the candidate preedit to the live tail. It sets `Candidate.preedit` for segmented-code display; do not assign `seg.prompt` from a Lua translator because Weasel 0.17.4 passes a const segment and the setter aborts candidate generation. Encoding stays in the candidate window by default: the pack must not force `inline_preedit` or `preedit_type: composition` in its schema or custom configs. If users enable composition preedit themselves, keep `Candidate.preedit` correct, including the live tail after a partial commit. The same files were copied to the current user directory `C:\Users\yc\AppData\Roaming\Rime`; after any further change, redeploy schema/config changes and restart the Rime client so Lua modules reload. Do not assume the Rime implementation is production-ready.
