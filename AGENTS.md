...
- Sentence input is disabled by default. When enabled, Core rebuilds a variable-length lattice from the entire raw code after every edit on a latest-generation-only background worker, so TSF key responses never wait for Beam Search; deterministic commit/selection actions synchronously ensure the latest result when necessary. A one-key segment is legal only when it is the whole input; other segments consume at least two keys including an optional selector (`;` selects rank 2, `'` selects rank 3, digits select an explicit rank). Semicolon and quote selectors enter the code only when their corresponding selection settings are enabled. Multi-character lexicon entries are legal edges. Core's compact n-gram model is authoritative fallback; neural reranking is asynchronous and generation-checked.
